...
printf "      dataset,n_corr, ni_gt, ir_gt,     n,ni_avg,ni_std,ni_min,ni_lwq,ni_med,ni_upq,ni_max,  rt_avg,  rt_std,  rt_min,  rt_lwq,  rt_med,  rt_upq,  rt_max,Efwd_avg,Efwd_std,Efwd_min,Efwd_lwq,Efwd_med,Efwd_upq,Efwd_max\n" | tee "${out_file}"
...
do
    ...
    do
        ...
        gt_file="${homogr_dir}/${img_base}.txt"
        printf "%13s," ${img_base} | tee -a "${out_file}"
        ${bin_dir}/nx-fit-homography -l "${img_left}"  -r "${img_right}" --validation-points "${vpts_file}" \
                  --ground-truth-r2l "${gt_file}" ${options} | tee -a "${out_file}"
    done
done
