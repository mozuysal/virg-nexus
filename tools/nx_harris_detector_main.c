...
static void local_radon_images(struct NXImage **lr_images,
                               struct NXImage *img, int sum_radius);

static void checker_score_image(struct NXImage *score_img,
...
int main(int argc, char** argv)
{
        ...
        if (NX_FALSE) {
                ...
                local_radon_images(lr_img, img, 20);
                checker_score_image(simg, lr_img);
...
void local_radon_images(struct NXImage **lr_images, struct NXImage *img, int sum_radius)
{
        for (int i = 0; i < 4; ++i) {
                if (!lr_images[i])
...
void checker_score_image(struct NXImage *score_img, struct NXImage **lr_images)
{
